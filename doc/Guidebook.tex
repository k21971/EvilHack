...
\Large Guidebook for {\it EvilHack\/}}

%.au
\author{Original version - Eric S. Raymond\\
(Edited and expanded for EvilHack by Keith Simpson and others)}
\date{May 30, 2026}
...
%.pg
You have just begun a game of {\it EvilHack}.  Your goal is to grab as much
...
%
\item[\bb{Druids}]%
are primal spellcasters of considerable power and versatility, who gain
their power through being at one with nature or through a connection to
a powerful deity or nature spirit.  Guardians of the wilderness, druids
see themselves less as masters of the natural order and more as an
extension of its will.
%.pg
%
\item[\bb{Healers}]%
...
\blist{}
\item[\bb{Aasimar}]%
are a race of celestial heritage, descended from the union of
mortals and divine beings.  They are devoted to the cause of law and
righteousness, and their celestial nature grants them various resistances
and abilities as they grow in experience.  An Aasimar that stays true to
their faith may receive extraordinary divine gifts, but those who stray
from the path of righteousness will find their celestial powers diminished
or even stripped away entirely.  Aasimar also possess a natural aura of
light that they can invoke at will.
%.pg
%
\item[\bb{Centaurs}]%
are a strong and proud race that have the upper body of a
humanoid, and the lower body of a large horse.  They are adept at physical
combat, especially with ranged weapons, and have the ability to jump great
distances when the need arises.  They can wear all armor that other typical
humanoid races can wear, except for boots.
%.pg
%
\item[\bb{Draugr}]%
are an undead race that are very similar to zombies but still
have free will and can reason, unlike their undead zombie brethren.  Draugr
are immune to things like poison, disease and death magic, and can revive
a number of times before being permanently destroyed.  However, they are
exteremely suscepible to fire, and are unable to learn or cast spells or
obtain intrinsic telepathy due to their almost-but-not-quite mindless
nature.
%.pg
%
\item[\bb{Drow}]%
(also known as `dark elves') are a dark-skinned sub-race of Elves
that shun the light whenever possible and prefer living underground.  They
share many physical attributes with their elven cousins, including their
aversion to iron.  Drow weapons and most armor are crafted out of a material
known as adamantine, whose quality can rivial that of mithril, but becomes
brittle in the light and can shatter with use unless shrouded in darkness.
%.pg
%
\item[\bb{Dwarves}]%
...
%
\item[\bb{Giants}]%
are humanoid in appearance, but are much larger.  Due to their
size, they are unable to wear many armor types found within the dungeon.
However, they can one-hand weapons that are two-handed for the other smaller
races, and they can carry and toss boulders as if they were rocks.  Giants
are a bit slower than other races, but their size and immense strength tend
...
%
\item[\bb{Hobbits}]%
(also known as Halflings) are one of the smallest humanoid
races, roughly half the size of a typical human.  Very quick and robust,
they excel at ranged weapon combat, especially with slings and small blades.
Hobbits are known for their love of food; they can innately sense if food
is safe to eat, and they can eat many times their weight before becoming
satiated.
%.pg
%
\item[\bb{Humans}]%
...
%
\item[\bb{Illithids}]%
are an ancient race that have natural psychic ability.
They are extremely intelligent and make excellent spellcasters, but are
one of the weakest races physically.  While Illithids can eat regular food,
they prefer to use their tentacle-like mouth to feed on other creatures
brains for sustenance.  Mind flayers are the wild cousins of the Illithid
race.  Illithids also possess innate telekinetic ability, allowing them
to push and pull objects and creatures with their mind.
%.pg
%
\item[\bb{Orcs}]%
...
%
\item[\bb{Tortles}]%
are a humanoid turtle race known for their strength and wisdom.
Their natural shells provide excellent protection against physical attacks,
but it also prevents them from wearing most types of armor.  They can also
hide inside their shells for an extended period of time, making them nearly
impervious to physical attacks.  Tortles are natural swimmers, and can
survive underwater indefinitely unaided.
%.pg
%
\item[\bb{Vampires}]%
are an extremely powerful undead race that enjoy immunity to
a great deal of various attack types, but are susceptible to fire and have
a severe aversion to silver.  Vampires do not eat regular food, but instead
drian the blood from their victims to survive.  They also have the ability
to shapechange into either a vampire bat, a warg, or a fog cloud at will.
\elist
...
%.pg
When {\it EvilHack\/}'s ancestor {\it rogue\/} first appeared, its screen
...
exception; {\it EvilHack\/} continues this fine tradition.  Unlike text
adventure games that accept commands in pseudo-English sentences and
explain the results in words, {\it EvilHack\/} commands are all one or two
...
%.pg
{\it EvilHack\/} can even be played by blind players, with the assistance of
Braille readers or speech synthesisers.  Instructions for configuring
{\it EvilHack\/} for the blind are included later in this document.

%.pg
{\it EvilHack\/} generates a new dungeon every time you play it; even the
authors still find it an entertaining and exciting game despite
having won several times.

%.pg
{\it EvilHack\/} offers a variety of display options.  The options available to
...
%.pg
In order to understand what is going on in {\it EvilHack}, first you must
understand what {\it EvilHack\/} is doing with the screen.  The {\it EvilHack\/}
screen replaces the ``You see \ldots'' descriptions of text adventure games.
Figure 1 is a sample of what a {\it EvilHack\/} screen might look like.
...
``{\tt --More--}'' on the top line, this means that {\it EvilHack\/} has
...
by {\it EvilHack\/}.  Other commands might require additional information, for
example a direction, or an object to be used.  For those commands that
require additional information, {\it EvilHack\/} will present you with either
...
%.lp ""
As you can see, the authors of {\it EvilHack\/}
...
%.lp
\item[\tb{\#forge}]
Combine two objects at a forge to create a new object,
or affix various gemstones to a weapon.
Autocompletes. Default key is `{\tt M-F}'.
\item[\tb{\#genocided}]
...
Requires adjacency (or telepathy with both you and your pet). Autocompletes.
% DON'T PANIC!
\item[\tb{\#panic}]
...
%.lp
%.lp
\item[\tb{\#removeimarkers}]
Remove remembered invisible monster markers (`{\tt I}') from the map.
Autocompletes. Default key is `{\tt \^{}U}'.
\item[\tb{\#ride}]
...
%.lp
\item[\tb{\#shapechange}]
Vampire's shapechanging ability.
Change into bat, fog cloud, or wolf form, or back to normal. Autocompletes.
\item[\tb{\#shell}]
Do a shell escape. Default key is `{\tt !}'.
%.lp
\item[\tb{\#shield}]
Toggle wearing a shield (sword and board) on or off. Autocompletes.
Default key is `{\tt \{}'.
%.lp
\item[\tb{\#showcolors}]
Show the terminal's color palette and detected color depth. Autocompletes.
%.lp
\item[\tb{\#sit}]
...
%.lp
\item[\tb{\#steal}]
Attempt to steal from an adjacent monster.
Rogues and Convicts only; must have bare hands. Autocompletes.
\item[\tb{\#suspend}]
...
%.lp
\item[\tb{\#telekinesis}]
Use telekinetic powers to push or pull an object or creature.
Autocompletes.
Default key is `{\tt \^{}K}'.
%.lp
\item[\tb{\#teleport}]
...
%.lp
\item[\tb{\#version}]
Print compile time options for this version of {\it EvilHack\/}.
...
%.lp
\item[\tb{\#wildshape}]
Druid's shapechanging ability.
Change into a creature form based on experience level. Autocompletes.
\item[\tb{\#wipe}]
...
In {\it NT, OS/2, PC\/ {\rm and} ST EvilHack},
...
option to have {\it EvilHack\/} combine them into meta\+key.
\blist{}
%.lp
\item[\tb{M-?}]
...
The `F' prefix also has other uses when interacting with
dungeon features and terrain.
Additionally, `F' suppresses automatic bonus attacks:
Monks with Master or higher skill in martial arts will not perform
random kick attacks, and characters wielding a shield at Basic or
higher skill will not attempt shield bashes.
This is useful against monsters with passive attacks that can
damage or destroy your boots or shield.
...
it up.  In {\it EvilHack}, this is accomplished automatically by walking over
...
%.pg
If you're carrying too many items, {\it EvilHack\/} will tell you so and you
...
%.pg
{\it EvilHack\/} will tell you how badly you have loaded yourself.
...
you want to use.  When {\it EvilHack\/} asks you to choose a particular object
...
%.pg
When you use one of these objects, if its effect is obvious, {\it EvilHack\/}
...
{\it EvilHack}.  Some of the more obscure weapons (such as the %
...
also an option,
{\it autoquiver},
which has {\it EvilHack\/} choose another item to automatically fill your
...
{\it EvilHack}.  Here is an (incomplete) list of the armor classes provided by
...
To use this feature on versions where {\it EvilHack\/}
mail delivery is triggered by electronic mail appearing in your system mailbox,
you must let {\it EvilHack\/} know where to look for new mail by setting the
...
file name of your favorite reader, so {\it EvilHack\/} can shell to it when you
read the scroll.
On versions of {\it EvilHack\/} where mail is randomly
...
%.pg
As if winning {\it EvilHack\/} were not difficult enough, certain players
...
%.pg
In {\it EvilHack}, a pacifist refuses to cause the death of any other monster
...
%.pg
Due to variations in personal tastes and conceptions of how {\it EvilHack\/}
should do things, there are options you can set to change how {\it EvilHack\/}
behaves.
...
file, or in the ``EVILHACKOPTIONS'' environment variable.
Some versions of {\it EvilHack\/} also have front-end programs that allow
...
\mbox{{``\%USERPROFILE\%\textbackslash EvilHack\textbackslash''}}. The
file may not exist, but it is a normal ASCII text file and can be created
with any text editor.
After running {\it EvilHack\/} for the first time, you should find a default
template for the configuration file named \mbox{``.evilhackrc.template''} in
\mbox{{``\%USERPROFILE\%\textbackslash EvilHack\textbackslash''}}.
If you had not created the configuration file, {\it EvilHack\/} will create
the configuration file for you using the default template file.

%.lp ""
On MS-DOS it is \mbox{``defaults.nh''} in the same folder as
\mbox{{\it evilhack.exe\/}}.\\
...
%.lp
\item[\bb{HACKDIR}]
Default location of files {\it EvilHack\/} needs. On Windows HACKDIR
defaults to the location of the {\it EvilHack.exe\/} or {\it EvilHackw.exe\/} file
...
See the ``Modifying {\it EvilHack\/} Symbols'' section.
%.pg
...
%.hn 2
\subsection*{Using the EVILHACKOPTIONS environment variable}

%.pg
The EVILHACKOPTIONS variable is a comma-separated list of initial
...
%.SD i
\begin{verbatim}
    setenv EVILHACKOPTIONS "color,\!leg,name:Blue Meanie,fruit:lime"
\end{verbatim}
%.ED
...
%.SD i
\begin{verbatim}
    EVILHACKOPTIONS="color,!leg,name:Blue Meanie,fruit:lime"
    export EVILHACKOPTIONS
\end{verbatim}
%.ED
...
%.pg
The EVILHACKOPTIONS value is effectively the same as a single OPTIONS
...
EVILHACKOPTIONS can be set to the full name of a configuration file you
want to use.
If that full name doesn't start with a slash, precede it with `{\tt @}'
(at-sign) to let EvilHack know that the rest is intended as a file name.
...
%.lp
\item[\ib{customcolors}]
Honor {\tt CUSTOMCOLOR} entries from the configuration file (default off).
Enable to apply per-glyph overrides loaded from the rc file on top of
{\it EvilHack's\/} built-in palette.  Has no effect when no
{\tt CUSTOMCOLOR} lines are configured.
See ``{\it Configuring Custom Glyph Colors\/}'' below.
%.lp
%.lp
...
{\it EvilHack\/} uses from time to time.  You should set this to something you
find more appetizing than slime mold.  Apples, oranges, pears, bananas, and
melons already exist in {\it EvilHack\/}, so don't use those.
%.lp
\item[\ib{gender}]
...
%.lp
\item[\ib{hallucinating}]
Start the character permanently hallucinating (default false).  Persistent.
\item[\ib{hawkname}]
Name your starting hawk (for example, ``{\tt hawkname:Betty}'').
Cannot be set with the `{\tt O}' command.
\item[\ib{help}]
...
%.lp
\item[\ib{hilite\verb+_+hidden\verb+_+stairs}]
Visually distinguish staircases that are concealed beneath one or more
objects, so that they are not overlooked (default off).
...
%.lp
\item[\ib{homunname}]
Name your starting homunculus (for example, ``{\tt homunname:Steve}'').
Cannot be set with the `{\tt O}' command.
\item[\ib{horsename}]
...
(Choosing its `{\tt A}' (Auto-select All) choice acts on every item and
skips the second menu; to avoid triggering that by accident, set
{\it paranoid\verb+_+confirmation:AutoAll\/} to require confirmation.)
...
%.lp
\item[\ib{news}]
Read the {\it EvilHack\/} news file, if present (default on).
...
prompting is desired.  The default is ``{\it paranoid\verb+_+confirmation:pray quit}''.
{\it quit} is always set unless specifically disabled with
``{\it OPTIONS=!paranoid\verb+_+confirmation}''.
%.sd
%.si
\newlength{\pcwidth}
...
\item[{\tt AutoAll}]
require `{\tt y}' to confirm the `{\tt A}' (Auto-select All) choice in the
object class menu shown for {\it menustyle:Full\/}, before acting on the
entire inventory or container;
...
%.lp
\item[\ib{pseudoname}]
Name your starting pseudodragon (for example, ``{\tt pseudoname:Carl}'').
Cannot be set with the `{\tt O}' command.
\item[\ib{pushweapon}]
...
%.lp
\item[\ib{ratname}]
Name your starting rat (for example, ``{\tt ratname:Squeak}'').
Cannot be set with the `{\tt O}' command.
\item[\ib{realtime}]
...
Persistent.
%.lp
\item[\ib{showmondamage}]
Whenever a monster you're attacking takes damage, show a message of the
damage taken, and the amount of hit points left.  Debug mode only.
%.lp
\item[\ib{showrace}]
...
%.lp
\item[\ib{spidername}]
Name your starting spider (for example, ``{\tt spidername:Phil}'').
Cannot be set with the `{\tt O}' command.
\item[\ib{standout}]
...
This option may be set to a {\it EvilHack\/} version level to suppress
...
Use ``{\tt symset:UTF8graphics}'' on UTF-8 terminals for Unicode line-drawing
characters and symbols.
When using IBMgraphics on a UTF-8 terminal, CP437 characters are
automatically converted to their Unicode equivalents.
%.lp
\item[\ib{time}]
...
%.lp
\item[\ib{toptenwin}]
Put the ending display in a {\it EvilHack\/} window instead of on stdout (default off).
Setting this option makes the score list visible when a windowing version
of {\it EvilHack\/} is started without a parent window, but it no longer leaves
...
%.lp
\item[\ib{truecolor}]
Allow the tty, curses, and Windows VT renderers to emit 24-bit RGB
escape sequences for {\tt CUSTOMCOLOR} entries when the terminal
advertises truecolor support ({\tt COLORTERM=truecolor},
{\tt COLORTERM=24bit}, or a {\tt *-direct} / {\tt *-truecolor}
{\tt TERM}).
Default is on.
The curses interface additionally requires ncurses 6.1 or later and
a TERM advertising the RGB terminfo capability (e.g.
{\tt xterm-direct}, {\tt tmux-direct}); on terminals without RGB
terminfo, curses falls back to the nearest 256-palette match.
%.lp
\item[\ib{verbose}]
...
non-comment line.
For a comma-separated list in EVILHACKOPTIONS or an OPTIONS line in a
...
If {\it EvilHack\/} can, it should display an ascii map.
%.lp
\item[\ib{color}]
If {\it EvilHack\/} can, it should display color for different monsters,
objects, and dungeon features.
On terminals that support 256 colors, {\it EvilHack\/} will automatically
use an extended color palette for dungeon features such as Gehennom walls,
various monsters, and terrain such as water, ice, and lava.
...
If {\it EvilHack\/} can, it should use a font by the chosen name for the
map window.
%.lp
\item[\ib{font\verb+_+menu}]
If {\it EvilHack\/} can, it should use a font by the chosen name for menu
windows.
%.lp
\item[\ib{font\verb+_+message}]
If {\it EvilHack\/} can, it should use a font by the chosen name for the message window.
%.lp
\item[\ib{font\verb+_+status}]
If {\it EvilHack\/} can, it should use a font by the chosen name for the status window.
%.lp
\item[\ib{font\verb+_+text}]
If {\it EvilHack\/} can, it should use a font by the chosen name for text windows.
%.lp
\item[\ib{font\verb+_+size\verb+_+map}]
If {\it EvilHack\/} can, it should use this size font for the map window.
%.lp
\item[\ib{font\verb+_+size\verb+_+menu}]
If {\it EvilHack\/} can, it  should use this size font for menu windows.
%.lp
\item[\ib{font\verb+_+size\verb+_+message}]
If {\it EvilHack\/} can, it should use this size font for the message window.
%.lp
\item[\ib{font\verb+_+size\verb+_+status}]
If {\it EvilHack\/} can, it should use this size font for the status window.
%.lp
\item[\ib{font\verb+_+size\verb+_+text}]
If {\it EvilHack\/} can, it should use this size font for text windows.
%.lp
\item[\ib{fullscreen}]
If {\it EvilHack\/} can, it should try and display on the entire screen rather than in a window.
%.lp
\item[\ib{guicolor}]
...
If {\it EvilHack\/} can, it should use a large font.
%.lp
\item[\ib{map\verb+_+mode}]
If {\it EvilHack\/} can, it should display the map in the manner specified.
%.lp
\item[\ib{player\verb+_+selection}]
If {\it EvilHack\/} can, it should pop up dialog boxes or use prompts for character selection.
%.lp
\item[\ib{popup\verb+_+dialog}]
If {\it EvilHack\/} can, it should pop up dialog boxes for input.
%.lp
\item[\ib{preload\verb+_+tiles}]
If {\it EvilHack\/} can, it should preload tiles into memory.
...
If {\it EvilHack\/} can, it should scroll the display by this number of cells
when the hero reaches the scroll\verb+_+margin.
%.lp
\item[\ib{scroll\verb+_+margin}]
If {\it EvilHack\/} can, it should scroll the display when the hero or cursor
is this number of cells away from the edge of the window.
%.lp
\item[\ib{selectsaved}]
If {\it EvilHack\/} can, it should display a menu of existing saved games for the player to
choose from at game startup, if it can. Not all ports support this option.
%.lp
\item[\ib{softkeyboard}]
If {\it EvilHack\/} can, it should display an onscreen keyboard.
Handhelds are most likely to support this option.
%.lp
\item[\ib{splash\verb+_+screen}]
If {\it EvilHack\/} can, it should display an opening splash screen when
...
If {\it EvilHack\/} can, it should display a tiled map if it can.
%.lp
...
%.lp
\item[\ib{underline\verb+_+peacefuls}]
Underline peaceful monsters shown on the map (default off).
\item[\ib{use\verb+_+256color}]
Use extended 256-color palette for materials, monsters and terrain
features on terminals that support it.
Disable this if colors appear wrong on your terminal.
Default is on.
See also {\tt CUSTOMCOLOR} for per-glyph color overrides and
{\it truecolor\/} for 24-bit RGB output.
%.lp
\item[\ib{use\verb+_+darkgray}]
Use bold black instead of blue for black glyphs (TTY only).
%.lp
\item[\ib{use\verb+_+inverse}]
If {\it EvilHack\/} can, it should display inverse when the game specifies it.
%.lp
\item[\ib{use\verb+_+menu\verb+_+glyphs}]
Display object glyphs alongside the entries in menus such as inventory
(default on).
\item[\ib{vary\verb+_+msgcount}]
If {\it EvilHack\/} can, it should display this number of messages at a time
...
If {\it EvilHack\/} can, it should display windows with the specified
...
%.lp
\item[\ib{wraptext}]
If {\it EvilHack\/} can, it should wrap long lines of text if they don't fit
...
Select an alternate way to handle keystrokes ({\it Win32 tty\/ EvilHack\/} only).
...
set to tell {\it EvilHack\/} to convert a two character sequence beginning with
...
abort the count by typing ESC will leave {\it EvilHack\/} waiting for another
...
compatible BIOS ROM (default off, {\it OS/2, PC\/ {\rm and} ST EvilHack\/} only).
%.lp
\item[\ib{flush}]
(default off, {\it Amiga EvilHack \/} only).
%.lp
\item[\ib{Macgraphics}]
(default on, {\it Mac EvilHack \/} only).
%.lp
\item[\ib{page\verb+_+wait}]
(default off, {\it Mac EvilHack \/} only).
%.lp
\item[\ib{rawio}]
Force raw (non-cbreak) mode for faster output and more
bulletproof input (MS-DOS sometimes treats `{\tt \^{}P}' as a printer toggle
without it) (default off, {\it OS/2, PC\/ {\rm and} ST EvilHack\/} only).
Note:  DEC Rainbows hang if this is turned on.
Cannot be set with the `{\tt O}' command.
%.lp
\item[\ib{soundcard}]
(default off, {\it PC EvilHack \/} only).
Cannot be set with the `{\tt O}' command.
%.lp
\item[\ib{subkeyvalue}]
({\it Win32 tty EvilHack \/} only).
May be used to alter the value of keystrokes that the operating system
returns to {\it EvilHack\/} to help compensate for international keyboard
issues.
OPTIONS=subkeyvalue:171/92
will return 92 to {\it EvilHack\/}, if 171 was originally going to be returned.
You can use multiple subkeyvalue statements in the configuration file
if needed.
Cannot be set with the `{\tt O}' command.
%.lp
\item[\ib{video}]
Set the video mode used ({\it PC\/ EvilHack\/} only).
...
(default 4-2-6-1-5-3-15-12-10-14-9-13-11, {\it PC\/ EvilHack\/} only).
...
(default dark normal light, {\it PC\/ EvilHack\/} only).
...
possible to compile {\it EvilHack\/} without regular expression support on
a platform where
there is no regular expression library. While this is not true of any modern
platform, if your {\it EvilHack\/} was built this way, patterns are instead glob
...
%.lp ""
Here's an example of message types using {\it EvilHack's\/} internal
...
On 256-color terminals, you can also use numeric color values from 0 to 255.
Extended colors automatically fall back to the nearest base color on
terminals that don't support them.
See the file {\tt colors256.md} in the documentation directory for
the complete palette reference.
...
%.lp ""
Here's an example of menu colors using {\it EvilHack's\/} internal
pattern matching facility:
...
%.lp
%.hn 2
\subsection*{Configuring Custom Glyph Colors}

%.pg
The {\tt CUSTOMCOLOR} configuration directive lets you override the
color of an individual monster, object, or dungeon feature without
recompiling.  {\it EvilHack's\/} hand-picked 256-color palette stays
the default; {\tt CUSTOMCOLOR} entries layer on top.

%.pg
The general syntax is:
\begin{verbatim}
    CUSTOMCOLOR=PREFIX:NAME/COLOR
\end{verbatim}

\blist{}
%.lp
\item[\ib{PREFIX}]
one of {\tt mon} (monster), {\tt obj} (object), or {\tt cmap}
(dungeon feature / map symbol);
%.lp
\item[\ib{NAME}]
the canonical name of the entity~--- the monster name from the
internal table, or the object's true (unidentified-shuffled) name.
For {\tt cmap} entries the name is either a {\tt defsyms} explanation
string, which colors every map feature that shares it (for example
{\tt wall} or {\tt water}), or a map symbol name with or without the
{\tt S\_} prefix (for example {\tt corr}, {\tt S\_corr}, {\tt lava}, or
{\tt litcorr}), which colors that single symbol.
Names are matched case-insensitively;
%.lp
\item[\ib{COLOR}]
either a 24-bit RGB value as {\tt \#RRGGBB} or as decimal
{\tt R-G-B} (each component 0--255), or one of the legacy 16 color
names ({\it black}, {\it red}, {\it green}, {\it brown}, {\it blue},
{\it magenta}, {\it cyan}, {\it gray}, {\it orange},
{\it bright-green}, {\it yellow}, {\it bright-blue},
{\it bright-magenta}, {\it bright-cyan}, {\it white}, or
{\it no-color}).
\elist
...
\begin{verbatim}
    CUSTOMCOLOR=mon:gold dragon/#FFD700
    CUSTOMCOLOR=mon:newt/bright-green
    CUSTOMCOLOR=obj:long sword/#C0C0C0
    # defsyms explanation
    CUSTOMCOLOR=cmap:molten lava/255-69-0
    # symbol name (with S_)
    CUSTOMCOLOR=cmap:S_corr/178-178-178
    # symbol name (without S_)
    CUSTOMCOLOR=cmap:throne/255-215-0
\end{verbatim}

%.pg
On terminals that advertise truecolor, RGB customcolors are emitted as
\verb+\033[38;2;R;G;Bm+ escapes.
On 256-color terminals, each entry falls back to its nearest 256-palette
match.
On legacy 16-color terminals, the 256 match further collapses to the
nearest base color.

%.pg
The {\it customcolors\/} option toggles the entire override layer at
runtime; with it off, the default {\it EvilHack\/} palette renders
unchanged.
...
%.pg
Your copy of {\it EvilHack\/} may have been compiled with support
...
You can also use numeric color values from 0 to 255 on 256-color terminals;
extended colors automatically fall back to the nearest base color on
terminals that don't support them.
...
Since EvilHack issues attribute requests sequentially (at least with
...
\begin{verbatim}
    ...
    OPTION=hilite_status: hitpoints/<25%/202
...
%.lp
%.hn 2
\subsection*{Modifying {\it EvilHack\/} Symbols}

%.pg
{\it EvilHack\/} can load entire symbol sets from the symbol file.
...
{\it EvilHack\/} includes a {\bf UTF8graphics} symbol set that uses Unicode
line-drawing characters for walls, and special characters such as
$\Psi$ (psi) for trees, $\Omega$ (omega) for altars, and
\dag{} (dagger) for graves.
To use it, add ``{\tt symset:UTF8graphics}'' to your configuration file.
Your terminal must support UTF-8 encoding.

When using IBMgraphics on a UTF-8 terminal, {\it EvilHack\/} automatically
converts CP437 characters to their Unicode equivalents, so IBM line
drawing and special characters display correctly without switching
to the UTF8graphics set.
...
Note that {\it EvilHack\/} escape-processes
...
\# & S\verb+_+darkroom                &	(dark room)\\
\verb@^@ & S\verb+_+dart\verb+_+trap              &	(dart trap)\\
\# & S\verb+_+deadtree                &	(dead tree)\\
...
\# & S\verb+_+ent                     &	(tree-like creature)\\
...
\verb@^@ & S\verb+_+ice\verb+_+trap               &     (ice trap)\\
...
\verb@}@ & S\verb+_+puddle                  &	(shallow water)\\
...
\verb@.@ & S\verb+_+sand                    &	(sand)\\
\verb@?@ & S\verb+_+scroll                  &	(scroll)\\
\verb@}@ & S\verb+_+sewage                  &	(sewage)\\
\# & S\verb+_+sink                    &	(sink)\\
\verb@z@ & S\verb+_+skeleton               &	(skeleton)\\
...
%.pg
%.hn 2
\subsection*{Configuring {\it EvilHack\/} for Play by the Blind}

%.pg
{\it EvilHack\/} can be set up to use only standard ASCII characters for making
maps of the dungeons. This makes the MS-DOS versions of {\it EvilHack\/}
completely
...
%.pg
{\it EvilHack\/} can also be compiled with support for sending the game messages
to an external program, such as a text-to-speech synthesizer. If the
``{\tt \#version}'' extended command shows ``external program as a
message handler'', your {\it EvilHack\/}
has been compiled with the capability. When compiling {\it EvilHack\/}
from source
...
Included within the symbol file of all official distributions of {\it EvilHack\/}
...
%.pg
If {\it EvilHack\/} is compiled with the SYSCF option, a system administrator
...
the other {\it EvilHack\/} support files.
...
%.pg
{\it EvilHack\/} maintains a list of the top scores or scorers on your machine,
...
%.pg
{\it EvilHack\/} is an intricate and difficult game.  Novices might falter
...
%.pg
{\it EvilHack\/} is the product of literally dozens of people's work.
...
%.pg
\medskip
{\it EvilHack\/} (this variant) was initially based on NetHack 3.6.2, created by
{\it Keith Simpson}. Development began on October 20, 2018, and was first
released to the public in April 2019. The GitHub repository for EvilHack
can be found at {\catcode`\#=11
\special{html:<a href="https://github.com/k21971/EvilHack/">}}
https:{\tt /}{\tt /}github.com{\tt /}k21971{\tt /}EvilHack{\tt /}.
{\catcode`\#=11
\special{html:</a>}}
