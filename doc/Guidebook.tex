...
%.lp
\item[\ib{truecolor}]
Allow the tty and Windows VT renderers to emit 24-bit RGB escape
sequences for {\tt CUSTOMCOLOR} entries when the terminal advertises
truecolor support ({\tt COLORTERM=truecolor}, {\tt COLORTERM=24bit}, or
a {\tt *-direct} / {\tt *-truecolor} {\tt TERM}).
Default is on.
The curses interface does not emit 24-bit; customcolor entries fall
back to the nearest 256-palette match instead.
%.lp
\item[\ib{verbose}]
...
On 256-color terminals (or in the curses windowport), each entry falls
back to its nearest 256-palette match.
