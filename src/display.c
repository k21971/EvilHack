...
int show;
{
    ...
    if (!cansee(x, y) && !lev->waslit) {
        ...
        if ((lev->typ == ROOM && glyph == cmap_to_glyph(S_room))
            ...
            || (lev->typ == DOOR && glyph == cmap_to_glyph(S_ndoor)))
    ...
    if ((trap = t_at(x, y)) != 0 && trap->tseen && !covers_traps(x, y)) {
        ...
        if (!lev->waslit
            && ((lev->glyph == cmap_to_glyph(S_room) && lev->typ == ROOM)
                ...
                || (lev->glyph == cmap_to_glyph(S_ndoor) && lev->typ == DOOR)))
    ...
    if (!can_reach_floor(FALSE)) {
        ...
        if (IS_ROCK(lev->typ)
            ...
            map_object(boulder, 1);
                   ...
                   && lev->glyph != cmap_to_glyph(S_ndoor)) {
            map_background(x, y, 1);
            ...
            if (lev->typ == CORR && lev->glyph == cmap_to_glyph(S_litcorr)
                ...
                show_glyph(x, y, lev->glyph = cmap_to_glyph(S_corr));
                     ...
                     && ((lev->typ == ROOM && lev->glyph == cmap_to_glyph(S_room))
                         ...
                         || (lev->typ == DOOR && lev->glyph == cmap_to_glyph(S_ndoor))))
        ...
        if (x == u.ux && y == u.uy) {
            ...
            if (mon && (see_it || (!worm_tail && Detect_monsters))) {
                ...
                display_monster(x, y, mon,
                                ...
                                worm_tail);
            } else if (mon && !mon_visible(mon) && has_erid(mon)
                       && mon_visible(ERID(mon)->mon_steed)) {
                display_monster(x, y, ERID(mon)->mon_steed, PHYSICALLY_SEEN, FALSE);
    ...
    /* Can't see the location. */
         ...
         */
            ...
            if (lev->glyph == cmap_to_glyph(S_litcorr) && lev->typ == CORR)
                ...
                show_glyph(x, y, lev->glyph = cmap_to_glyph(DARKROOMSYM));
            else if (lev->glyph == cmap_to_glyph(S_ndoor) && lev->typ == DOOR)
                show_glyph(x, y, lev->glyph = cmap_to_glyph(DARKROOMSYM));
            else
                goto show_mem;
        } else {
    ...
    if (iflags.use_background_glyph && lev->seenv != 0
        ...
        if (!cansee(x, y) && (!lev->waslit || flags.dark_room)) {
            ...
            if (lev->typ == CORR && idx == S_litcorr)
                idx = S_corr;
            else if (lev->typ == DOOR && idx == S_ndoor)
...
static const char *type_names[MAX_TYPE] = {
    ...
    "MAGIC_CHEST", "FOUNTAIN", "THRONE", "SINK", "GRAVE", "ALTAR", "ICE", "GRASS",
    "SAND", "DRAWBRIDGE_DOWN", "AIR", "CLOUD", "PUDDLE", "SEWAGE"
};
