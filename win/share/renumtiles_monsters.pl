...
my %commands = (
    'd' => 'debug mode; parse monsters.txt to stdout instead of updating',
);
...
my $tilecount = 0;
my $outfile = $debug ? "-" : "monsters.txt";
my $infile = $debug ? "monsters.txt" : "monsters.bak";


unless ($debug) {
    if (-e "$infile") { die "something didn't clean up monsters.bak from last time; stopping\n"; }
    rename($outfile,$infile) or die "couldn't move monsters.txt to monsters.bak; stopping\n";
}
