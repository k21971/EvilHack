...
my %commands = (
    'd' => 'debug mode; parse other.txt to stdout instead of updating',
);
...
my $tilecount = 0;
my $outfile = $debug ? "-" : "other.txt";
my $infile = $debug ? "other.txt" : "other.bak";


unless ($debug) {
    if (-e "$infile") { die "something didn't clean up other.bak from last time; stopping\n"; }
    rename($outfile,$infile) or die "couldn't move other.txt to other.bak; stopping\n";
}
